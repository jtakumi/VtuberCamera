...
class ARSceneManager @Inject constructor() {
    ...
    fun updateLighting(lightEstimate: LightEstimate) {
        ...
        try {
            ...
            Log.d(TAG, "Updated lighting - Intensity: $currentLightIntensity, Temp: $currentColorTemperature")
     ...
     */
    fun configureCameraForAR(width: Int, height: Int, nearPlane: Float = 0.1f, farPlane: Float = 100.0f) {
        try {
     ...
     */
    private fun estimateColorTemperature(r: Float, g: Float, b: Float): Float {
    ...
    private fun colorTemperatureToRGB(temperature: Float): FloatArray {
        ...
        val red = when {
            temp <= 66 -> 1.0f
                ...
                val r = temp - 60
                val red = 329.698727446f * Math.pow(r.toDouble(), -0.1332047592).toFloat()
        ...
        val green = when {
            temp <= 66 -> {
                val g = 99.4708025861f * Math.log(temp.toDouble()).toFloat() - 161.1195681661f
                (g / 255.0f).coerceIn(0.0f, 1.0f)
            }
            else -> {
                val g = temp - 60
                val green = 288.1221695283f * Math.pow(g.toDouble(), -0.0755148492).toFloat()
        ...
        val blue = when {
            ...
            temp <= 19 -> 0.0f
                ...
                val b = temp - 10
                val blue = 138.5177312231f * Math.log(b.toDouble()).toFloat() - 305.0447927307f
